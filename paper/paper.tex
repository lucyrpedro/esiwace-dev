...
To solve a computational problem, we can consider describing the necessary steps from the start to the scientific insight generated by the workflow.
...
\hline
& \parbox{5.5cm}{\centering \textbf{Input}} & \parbox{3.5cm}{\centering \textbf{Tasks}} & \parbox{3.5cm}{\centering \textbf{Products}} \\ \hline \hline

Description & Information inserted in the workflow. & The tasks specified by the user. & Possible outcomes of the tasks. \\ \hline

File & \texttt{input.ds} & \texttt{tasks.ds} & \texttt{output.ds} \\ \hline
...
\item[Input] The information inserted in the workflow.

\item[Tasks] The tasks specified by the user.

\item[Products] Any of the possible outcomes of the tasks.

For simplicity, we will call these datasets as \texttt{product-workflow.ds}. In this specific example, it is composed by the datasets \texttt{product1.ds}, \texttt{product2.ds}, and \texttt{product3.ds}.
...
\begin{enumerate}
  ...
  \item \textbf{Slurm}\ \ Once a task can be executed (dependencies are fulfiled), Cylc generates on the fly a Slurm script with the required metadata for Slurm, invokes the user-defined script for that task, and runs Slurm to queue the process.
