...
Alternatively (\figref{fig:lifecycle2}, it could be stored on the \textbf{scratch} file system immediately and accessed there.
However, that would require that the last read access happens before files on \textbf{scratch} are automatically removed.
The last scenario (\figref{fig:lifecycle3}) presents the case where data is created on \textbf{work} by a task. Still, then it is copied to a \textbf{local} node to allow read accesses of subsequent tasks which might be beneficial for small random accesses.
...
With this information, the I/O scheduler can make the placement and migration decisions for individual datasets and manage their relocation during their life cycle, as discussed in \figref{sec:datamanagement}.\lr{Check ref.}
...
\fig{width=0.8\columnwidth}{pic/cycle4}{Four stages workflow example}
...
The scenario presented in \figref{fig:cycle1} shows the same cycle of \figref{fig:cycle-io-dep}, but  extended information about datasets is added.
