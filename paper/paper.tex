...
\lr{This next paragraph should be migrate to 4.3 and again mentioned in 5.3.}
...
The idea here is to embrace the concept that tasks dependencies are really imposed by datasets dependencies. Once we can insert the datasets explicitly in the workflow, it is easier to see that many optimisations regarding storage can be made.

\lr{The end of the paragraph is all about motivation...}


\subsection{Extended Workflow Description}

The user can make decisions about storage, and the processing of the datasets/tasks. S/he will insert this information as an additional file to the Cylc workflow management.
In fact, the user is already doing that, but he is doing just to make the workflow feasible. Here, we are thinking about simplifying the information the user is already providing. So, instead of giving the complete workflow to Cylc, we are expecting the user to provide straightforward information, like:

\lr{Now this part contradicts the previous part, because it talks about only one dataset per task. I wish we could keep something that high-level, but it might not be possible for the text and, worse, it might not be feasible for us.}

\lr{Checking 4.2, we shoukd have something concrete here. High-level and concrete would be amazing.}
...
\lr{Does Cylc have the information about the system? If so, we can just use it. If not, how to get it for our purpose? And how it's done now?}


\subsection{Smarter Scheduling}

\lr{It's clear that I was mixing ESDM and EIOS in a very confusing way. :( The main information that should be here is, imho, in the next section. Consider just drop it.}
...
\lr{Note that this is the first mention to ESDM in the whole text! I think the E was for ESiWACE, but I think it has to be ESDM.}
...
\begin{enumerate}
  ...
  \item \textbf{EIOS} The proposed system reads the information about the workflow (original and I/O suit files) and acts depending on the stage of the execution. Two options are possible: we could be preparing the execution in Cylc, or we are already inside a script.
  \lr{It's not clear to me the consequences of having to options in calling EIOS. It seems the next text is just about one of them.}
  EIOS loads the \texttt{esdm.conf} file that contains information about the available technology in the data centre and it provides the information on how to prioritise I/O targets.
  A data management plan may run as a service and purge or migrate data according to the user life cycle specification and the novelty information from EIOS.
...
\lr{Any novelty here? It's better say it's not than say nothing.}
  ...
  Potentially, we can also generate an \texttt{esdm.conf} file to reflect the specific hardware we have available to run the specific application.

\lr{You already talked about esdm.conf in 2, almost the same information. And 6 will use it, so it has to be available.}
  ...
  \lr{Now I'm not sure I'm comfortable with ESDM being enforced here. Is the replacement mandatory to our model works? I don't think so. It's nice, sure, but it seems tie-in sale, which is not good. Also note that I commented a lost sentence here, that can be used somewhere else.}
